package com.flyjingfish.test_app;
...
public class MyTestApp extends Application {
    ...
    public static String[] check13ReadExternalStorage(String[] permissions) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU && (Arrays.asList(permissions).contains(Manifest.permission.READ_EXTERNAL_STORAGE)||Arrays.asList(permissions).contains(Manifest.permission.WRITE_EXTERNAL_STORAGE))) {
